...
def crypto_dat_file_creation(json_input, context):
    ...
    # Output to a file
    datFile = 'ransomware.dat'
    tempLocation = '/tmp/'
    datTemp = tempLocation + datFile
    try:
        f = open(datTemp, 'w')
        f.write(extensions)
        f.close()
    except:
        print("Unable to save DAT file")

    # Upload to S3
    datUploadLocation = 'dat/'
    datUpload = datUploadLocation + datFile
    try:
        datFileUpload = bucket.upload_file(datTemp, datUpload, {'ACL': 'public-read'})
    except:
        print("Unable to upload DAT file to S3")